...
export function Sidebar() {
  ...
  const { projects, removeProject } = useTodoStore()
          ...
          return (
            ...
            >
              <Link
                ...
                onClick={closeDrawer}
                className={cn(
                  "flex flex-1 items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm font-medium transition-all border-l-4",
                  !isActive && "border-transparent hover:border-[var(--project-color)] hover:bg-[var(--project-color)]/10",
                  isActive && "nav-item-active rounded-l-none"
                )}
              >
                <ProjectIcon name={project.icon} size={16} className="text-[var(--project-color)]" />
